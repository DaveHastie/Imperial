...
int main(int argc, char *  argv[])
{
  ...
  string resumeName=Get_stddzed_name(path_name_out,
                                     n_sweeps,
                                     Name_number,
                                     "resume",
                                     Extension_out);
  fstream f_resume;
  string rngFileName=Get_stddzed_name(path_name_out,
                                       n_sweeps,
                                       Name_number,
                                       "random",
                                       ".rng");
  ...
  if(!postProcessOnly){
    for(sweep=resumeSweep+1;sweep<n_sweeps+1;sweep++){
      ...
      if(timeLimit>0){
        ...
        saveResumeFile(f_resume,f_rng,sweep,g,shuffleYIndex,nY,cum_g,count_g,vect_RMSE,t_tun,My_g_AdMH->ls,My_DR,
