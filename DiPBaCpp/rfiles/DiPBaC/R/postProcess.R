...
plotRiskProfile<-function(riskProfObj,outFile,showRelativeRisk=F,orderBy=NULL,whichClusters=NULL,whichCovariates=NULL,useProfileStar=F){
	...
	if(includeResponse){
		...
		plotMax<-2*max(riskUpper)-riskMedian
		
		# Get the plot colors
		riskColor<-ifelse(riskLower>rep(riskMedian,nClusters),"high",
		                  ifelse(riskUpper<rep(riskMedian,nClusters),"low","avg"))
	}else{
	...
	if(includeResponse){
		if(yModel=='Categorical'){
			...
			plotObj<-plotObj+geom_hline(aes(x=as.factor(cluster),y=prob,yintercept=medianProb))
			...
			plotObj<-plotObj+labs(title=ifelse(showRelativeRisk,'Relative Risk','Risk'))+theme(plot.title=element_text(size=10))
	    ...
	    riskDF<-data.frame("risk"=c(),"cluster"=c(),"medianRisk"=c(),
	    ...
	    for(c in whichClusters){
	      ...
	      riskDF<-rbind(riskDF,data.frame("risk"=plotRisk,"cluster"=rep(c,nPoints),
	                                      "medianRisk"=rep(riskMedian,nPoints),
			...
			plotObj<-ggplot(riskDF)
			plotObj<-plotObj+geom_hline(aes(x=as.factor(cluster),y=risk,yintercept=medianRisk))
	...
	if(!is.null(yModel)){
		...
		plotObj<-plotObj+scale_colour_manual(values = c(high ="#CC0033",low ="#0066CC", avg ="#33CC66"))+
			theme(legend.position="none")
		plotObj<-plotObj+labs(title='Empirical Data')+theme(plot.title=element_text(size=10))
	...
	plotObj<-plotObj+labs(title="Size")+theme(plot.title=element_text(size=10))
	plotObj<-plotObj+theme(axis.title.x=element_text(size=10),axis.title.y=element_text(size=10,angle=90))
	...
	for(j in 1:nCovariates){
		if(xModel=='Discrete'){
			profileDF<-data.frame("prob"=c(),"cluster"=c(),"category"=c(),"medianProb"=c(),
				"lowerProb"=c(),"upperProb"=c(),"fillColor"=c())
        ...
        probLower<-apply(probMat,2,quantile,0.05)
			...
			plotObj<-plotObj+labs(title=covNames[j])+theme(plot.title=element_text(size=10))
